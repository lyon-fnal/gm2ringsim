...
gm2ringsim::Gm2ModularPhysicsList::Gm2ModularPhysicsList(G4VModularPhysicsList *mPL) : 
  G4VModularPhysicsList(*mPL),
  verboseLevel_(1),
  decayStatus_(decay_init),
  decayPhysicsList_(new G4DecayPhysics),
  physics_(new PhysicsListVector) 
...
void gm2ringsim::Gm2ModularPhysicsList::ConstructParticle(){
...
void gm2ringsim::Gm2ModularPhysicsList::ConstructProcess(){
  printf("We're in gm2ringsim::Gm2ModularPhysicsList::ConstructProcess()\n");
...
void gm2ringsim::Gm2ModularPhysicsList::SetCuts(){ 
  SetCutsWithDefault(); 
}

G4String gm2ringsim::Gm2ModularPhysicsList::currentDecay(){
  G4String t;
  switch( decayStatus_ ){
  case decay_init:
    t = "initializing";
...
G4int gm2ringsim::Gm2ModularPhysicsList::verboseLevel(G4int level) { 
...
void gm2ringsim::Gm2ModularPhysicsList::enableSMDecay(){
...
template<class T> void gm2ringsim::Gm2ModularPhysicsList::muonDecay(){
  G4ProcessManager *manager;
  G4VProcess *process;
...
void gm2ringsim::Gm2ModularPhysicsList::unpolDecayChannel(){
  G4DecayTable *dt;
...
void gm2ringsim::Gm2ModularPhysicsList::polDecayChannel(){
  G4DecayTable *dt;
...
void gm2ringsim::Gm2ModularPhysicsList::enableStepLimiter(){
  theParticleIterator->reset();

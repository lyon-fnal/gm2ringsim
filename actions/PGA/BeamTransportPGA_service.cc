...
// Get the PGA header
#include "gm2ringsim/actions/PGA/BeamTransportPGA_service.hh"
...
// Constructor
gm2ringsim::BeamTransportPGA::BeamTransportPGA(fhicl::ParameterSet const& p, art::ActivityRegistry&) :
  ...
  UseConstBeta_(p.get<bool>("UseConstBeta", false)),
  UseDispersion_(p.get<bool>("UseDispersion", true)),
  UseFNALt0_(p.get<bool>("UseFNALt0", true)),
  ...
  Debug_ (p.get<bool>("Debug", false))
{
  ...
  if ( StartPerfect_ ) { Kick_ = 0.0; }

  if ( Debug_ ) {
    G4cout << "================== BeamTransportPGA ==================" << G4endl;
    ...
    G4cout << "  UseDispersion       " << UseDispersion_ << G4endl;
    G4cout << "  UseFNALt0           " << UseFNALt0_ << G4endl;
...
gm2ringsim::BeamTransportPGA::~BeamTransportPGA(){
  delete gps_;
}


// initialize
void gm2ringsim::BeamTransportPGA::initialize() {
  
  //  particleGun_ = new G4ParticleGun(1);
  // Read the parameter set and figure out what sort of gun to use
  gps_ = new G2BeamTransportSource();  
  ...
  gps_ -> SetUseDispersion(UseDispersion_);
  gps_ -> SetUseFNALt0(UseFNALt0_);
...
// EndOfBeamTransportPGA
void gm2ringsim::BeamTransportPGA::generatePrimaries(G4Event* evt) {
...
DEFINE_ART_SERVICE(BeamTransportPGA)

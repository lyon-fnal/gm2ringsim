...
#define MAXRAD 51            // num of radial bins
...
int first_time_bin = 1000; //limits of analysis
int last_time_bin = 15000;
...
void fasrof_run(double pw, int t0binsize, string outputfilebase)
{
  ...
  for ( int i = 0; i < 4; i++ ) {
    ...
    int pos = simplebase.find(removes[i]);
    //cout << "  POS = " << pos << endl;
    if ( pos != string::npos ) {
      string str1, str2;
...
//   cout << "nt0bins = " << nt0bins << endl;
  ...
  for(int n=0;n<NRAD;n++) {
    for(int k=0;k<NRAD;k++) {
      sdw[n][k] = 0.0;
      if(n==k) sdw[n][k] = lambda*2.0;
      if(n==k-1) sdw[n][k] = -1.0*lambda;
      if(n==k+1) sdw[n][k] = -1.0*lambda;
  ...
  //----------------------------------
  char hisname[80];
  TH1D *hfr = (TH1D*)f1->Get("hSignalAll");
  if ( hfr == NULL ) {
    cout << "Could not find " << "hSignalAll." << endl;
    return;
  }
  ...
  TH1D *hum = new TH1D("hum","hum",50000,0.0,50000.0);
  hum->SetXTitle("time [ns]");
  TH2D *humspec = new TH2D("humspec","humspec",50000,0.0,50000.0, NRAD, -0.5*NRAD/9.0, +0.5*NRAD/9.0);
  humspec->SetXTitle("time [ns]");
  humspec->SetYTitle("radius [cm]");
  TH1D *hcheck = new TH1D("hcheck","hcheck",50000,0.0,50000.0);
  hcheck->SetXTitle("time [ns]");
  TH1D *hspread = new TH1D("hspread","hspread",50000,0.0,50000.0);
  hspread->SetXTitle("time bin (radial bin 20)");
  TH1D *hspread10 = new TH1D("hspread10","hspread10",50000,0.0,50000.0);
  hspread10->SetXTitle("time bin (radial bin 1)");
  TH1D *hspread30 = new TH1D("hspread30","hspread30",50000,0.0,50000.0);
  hspread30->SetXTitle("time bin (radial bin 39)");		       
  //TH1D *hspread5 = new TH1D("hspread5","hspread5",50000,0.0,50000.0);
  //TH1D *hspread20 = new TH1D("hspread20","hspread20",50000,0.0,50000.0);
  //TH1D *hspread35 = new TH1D("hspread35","hspread35",50000,0.0,50000.0);
  hum->SetLineColor(4);
  ...
  for(int irep=1;irep<=MAXREP;irep++) {
	...
	//cout << "Gen[" << j << "] = " << genspec[j] << endl;
      ...
      double chi2gen = hgen->Chi2Test(hfr2, "WW, CHI2/NDF");
      double pval = hgen->Chi2Test(hfr2, "WW");
      cout << "  Integral(Data)  = " << hfr2->Integral() << endl;
      cout << "  Integral(MC)    = " << hgen->Integral() << endl;
      cout << "  Chi2/Ndof (gen) = " << chi2gen << endl;
      out << chi2gen << endl;
      out << pval << endl;
      double ks = hgen->KolmogorovTest(hfr2);
      cout << "         KS (gen) = " << ks << endl;
      out << ks << endl;
      cout << endl;
...
// 	}
  ...
  //hcheck->Draw("same");
  int nn = series;
//sprintf(outfile,"fasrofwide%02d.root",nn);
//printf("file is %s \n",outfile);
  ...
  if ( gr_Data_Signal ) { gr_Data_Signal->Write("g_Data_Signal"); }
  if ( gr_MC_Signal )   { gr_Data_Signal->Write("g_MC_Signal"); }
